...
def generate(tokenizer, model, sentences, label):
    with torch.no_grad():
        for idx in range(sentences):  # sentence = 10
            finished = False
...
def load_gptdata():
    ...
    data = []
    file = open('/data/yuanmq/hybridnet/gptbertymq_eff.txt')
...
def train(loader, model, optimizer, opt, cap_vocab, cms_vocab):
    ...
    MODEL_NAME = '/data/yuanmq/hybridnet/mymodel_eff1.pt.pt'     #opt['model_name']
    ...
    for epoch in range(1, opt['epochs']+1):
        ...
        for data in loader:
            ...
            for j in range(len(data['video_ids'])):
                ...
                i = 0
                xxx = 0
                for i in range(31367):
                    if videoid in a[i][0]:
                        xxx =i
                        cms_gpt[j] = a[i][1]
                if  xxx == 0:
                    LABEL = data['caption'][j] + ' and the result is:'
            ...
            loss = cap_loss + 0*cms_int_loss + 1*cms_eff_loss + 0*cms_att_loss
